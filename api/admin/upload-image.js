...
export default async function handler(req, res) {
  ...
  if (!adminToken) {
    return res.status(500).json({ ok: false, error: 'ADMIN_TOKEN missing' });
  }
  if (!hasUpload) {
    return res.status(500).json({ ok: false, error: 'Додайте GITHUB_TOKEN або IMGBB_API_KEY у Vercel' });
  }
