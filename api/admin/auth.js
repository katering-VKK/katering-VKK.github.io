export default async function handler(req, res) {
  ...
  // GET: перевірка чи ADMIN_TOKEN налаштований (для дебагу)
  if (req.method === 'GET') {
    return res.status(200).json({
      configured: !!process.env.ADMIN_TOKEN,
      hint: process.env.ADMIN_TOKEN ? 'Token is set' : 'ADMIN_TOKEN missing in Vercel env',
    });
  }
  ...
  if (!adminToken) return res.status(500).json({ ok: false, error: 'Admin not configured. Add ADMIN_TOKEN in Vercel → Settings → Environment Variables, then Redeploy.' });
  ...
  if (!password) {
    return res.status(401).json({ ok: false, error: 'Введіть пароль' });
  }
